...
AdditionalShaders = {
  ...
  'gauss': {
    ...
    fragmentShader: "precision highp float;	 			uniform sampler2D tImg;  			uniform sampler2D tGaussOffsets;			uniform sampler2D tDepth;			uniform vec2 v2ImageSize;  			uniform vec2 v2SamplingDir;			                           			varying vec2 v2uv;         			                                                 			float fGaussStep = 1.0 / ( KERNEL_SIZE + 1.0 );                                          			float fGaussCenterOffset = fGaussStep / 2.0;                                                 						float offset( float x )			{				return texture2D( tGaussOffsets, vec2( x * fGaussStep + fGaussCenterOffset, fGaussStep + fGaussCenterOffset ) ).x;			}			                                                                                         			float gauss( float x )                                                                   			{                                                                                        			   return texture2D( tGaussOffsets, vec2( x * fGaussStep + fGaussCenterOffset, fGaussCenterOffset ) ).x;			}			                                                                                         			vec4 blurGauss()                                                                         			{                                                                                        			   float step;                                                                           			   vec2 centerOffset;                                                                    			                                                                                         			   if( v2SamplingDir.x > 0.0 )                                                           			   {                                                                                     			   		step = 1.0 / v2ImageSize.x;                                                      			   		centerOffset = vec2( 0.5, 0.0 ) * step;                                          			   }                                                                                     			                                                                                         			   else                                                                                  			   {                                                                                     			   		step = 1.0 / v2ImageSize.y;                                                      			   		centerOffset = vec2( 0.0, 0.5 ) * step;                                          			   }                                                                                     			                                                    			   vec4 v4Color = texture2D( tImg, v2uv + centerOffset ) * gauss( 0.0 );                                            			   for( float i = 1.0; i <= KERNEL_SIZE; ++i )                                  			   {                                                                                     			   		vec2 v2SamplingPos = ( v2uv + centerOffset ) + v2SamplingDir * offset( i ) * step;					v4Color += texture2D( tImg, v2SamplingPos ) * gauss( i );										v2SamplingPos = ( v2uv + centerOffset ) - v2SamplingDir * offset( i ) * step;                                                                                                                           					v4Color += texture2D( tImg, v2SamplingPos ) * gauss( i );			   }                                                                                     			   	                                                                                     			   return v4Color;                                                            			}						highp float unpackFloatRGB( vec3 vValue )                            			{                                                                                 				highp vec3 v3UnpackValues = vec3( 1.0 / ( 256.0 * 256.0 ), 1.0 / 256.0, 1.0 );				return dot( vValue, v3UnpackValues );                                         			}                                                                                   			                                                                                         			void main()                                                                              			{   				/*float fDepth = unpackFloatRGB( texture2D( tDepth, v2uv ).xyz );*/			    gl_FragColor = vec4( blurGauss().xyz, 1.0 );			}"
  }
};